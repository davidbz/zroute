...
pub const Resolver = union(enum) {
    ...
    /// Resolves `host` into up to `out.len` addresses. See `dialer.connect`
    /// for the policy-gated connect built on top of this.
    pub fn resolveAddresses(r: Resolver, io: Io, host: HostName, out: []net.IpAddress) ![]net.IpAddress {
        return switch (r) {
