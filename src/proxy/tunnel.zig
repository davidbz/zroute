...
pub fn handle(
    ...
    if (!egress_policy.allowsConnectPort(target.port)) {
        try egress.denyEgress(request, trace_id, slot, "connect port not allowlisted", target.host, target.port);
        return;
    }
    ...
    const upstream = resolver.connect(host_name, io, target.port, .{
        ...
        if (e == error.EgressDenied) {
            try egress.denyEgress(request, trace_id, slot, "egress denied", target.host, target.port);
            return;
        }
...
fn splice(
    ...
    var future = Io.concurrent(io, pump, .{ client_in, upstream_out, upstream_stream, io, trace_id, slot, "client->upstream" }) catch |e| {
    ...
    pump(upstream_in, client_out, client_stream, io, trace_id, slot, "upstream->client");
    future.await(io);
}
...
/// silently here; it's now logged.
fn pump(r: *Io.Reader, w: *Io.Writer, peer: net.Stream, io: Io, trace_id: TraceId, slot: u32, direction: []const u8) void {
